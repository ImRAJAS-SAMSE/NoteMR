...
class BlipITM(BlipBase):
    ...
    def __init__(self, image_encoder, text_encoder, embed_dim=256, max_txt_len=256):
        super().__init__()
...
def compute_gradcam(model, visual_input, text_input, tokenized_text, block_num=6):
    ...
    with torch.no_grad():
        ...
        cams = cams[:, :, :, 1:].reshape(visual_input.size(0), 12, -1, 24, 24) * mask      #1,12,35,577     1,12,13,577
        grads = (
